...
// ---------------- Auth de tienda ----------------
if (!STORE_KEY) {
  console.warn("[kapohs] ⚠️  STORE_KEY no configurado — /tienda abierto (modo desarrollo). Define STORE_KEY en producción.");
}
...
db.init().then(() => {
  app.listen(PORT, () => {
    console.log(`[kapohs] Servidor corriendo en http://localhost:${PORT}`);
    console.log(`[kapohs] Clientes: http://localhost:${PORT}/`);
    console.log(`[kapohs] Tienda:   http://localhost:${PORT}/tienda`);
  });
}).catch((err) => {
  console.error("[kapohs] ❌ No se pudo inicializar la base de datos:", err.message);
  console.error("[kapohs] Revisa DATABASE_URL (Postgres) o los permisos del archivo kapohs-kitchen.db (SQLite).");
  process.exit(1);
});
